...
void FFTWOLATestAudio::prepareToPlay(double sampleRate, int max_samplesPerBlock, int max_channels)
{
...
void FFTWOLATestAudio::prepareParameter(std::unique_ptr<juce::AudioProcessorValueTreeState> &vts)
{
    juce::ignoreUnused(vts);
}


FFTWOLATestGUI::FFTWOLATestGUI(FFTWOLATestAudioProcessor& p, juce::AudioProcessorValueTreeState& apvts)
:m_processor(p) ,m_apvts(apvts)
{
    
}

void FFTWOLATestGUI::paint(juce::Graphics &g)
{
    ...
    juce::String text2display = "FFTWOLATest V " + juce::String(PLUGIN_VERSION_MAJOR) + "." + juce::String(PLUGIN_VERSION_MINOR) + "." + juce::String(PLUGIN_VERSION_PATCH);
    g.drawFittedText (text2display, getLocalBounds(), juce::Justification::bottomLeft, 1);

}

void FFTWOLATestGUI::resized()
{
